...
os::cmd::expect_success 'oc new-project metric'

os::cmd::expect_success 'oc create -f "$TEST_DIR"/spark-metrics-template.yaml'

os::cmd::expect_success 'oc new-app --template=spark -p MASTER_NAME=master -p WORKER_NAME=worker -p SPARK_METRICS_ON=jolokia'
...
os::cmd::expect_success 'oc delete project metric'
